...
// drawTile(x, y, *tile) will use the game grid of a 16x11 grid where 1x1 is the top left
// to draw the *tile at (x, y), 16x11 is the bottom right
void drawTile(short x, short y, const long long* tile) {
  if (x > 16)
    x = 16;
  if (x < 1)
    x = 1;
  ...
  if (y > 11)
    y = 11;
  // x * 32 + RIGHT_SHIFT --> tune based on monitor
  drawTileFine(x * 32, y * 32, tile);
}

// drawTileFine(x, y, *tile) will draw a tile at any position on the screen
void drawTileFine(short x, short y, const long long* tile) {
  int start_pos = ((640 * (y + V_OFFSET)) + (x + H_OFFSET));
  // draws multiple rows
  for (int j = 0; j < 32; j += 2) {
    // draws a full row across 2 rows
    for (int i = 0; i < 2; i++) {
      char* vga_start_pt = &vga_data_array[(start_pos + 16 * i + 640 * j) >> 1];
      memcpy(vga_start_pt, tile + i + j, 8); // copies from design array
      vga_start_pt = &vga_data_array[(start_pos + 16 * i + 640 * (j + 1)) >> 1];
      memcpy(vga_start_pt, tile + i + j, 8); // copies from design array
    }
  }
}

// drawTileMono(x, y, color) will use the game grid of a 16x11 grid where 1x1 is the top left
// to draw a monotone colored tile at (x, y)
void drawTileMono(short x, short y, long long color) {
  if (x > 16)
    x = 16;
  if (x < 1)
    x = 1;
  ...
  if (y > 11)
    y = 11;
  drawTileMonoFine(x * 32, y * 32, color);
}

// drawTileMonoFine(x, y, color) will draw a color tile at (x, y)
void drawTileMonoFine(short x, short y, long long color) {
  int start_pos = ((640 * (y + V_OFFSET)) + (x + H_OFFSET));
  ...
  for (int j = 0; j < 32; j += 2) {
    // draws a full row across 2 rows
    for (int i = 0; i < 2; i++) {
      memcpy(&vga_data_array[(start_pos + 16 * i + 640 * j) >> 1], &long_mask, 8);       // copies from design array
      memcpy(&vga_data_array[(start_pos + 16 * i + 640 * (j + 1)) >> 1], &long_mask, 8); // copies from design array
    }
  }
}

// drawTop(x, y, *des, *mask) will draw, at (x, y) using the 16x11 grid where 1x1 is top left, *des with *mask such that the mask determines what
// is written and what is kept from the background
void drawTop(short x, short y, const long long* des, const char* mask) {
  if (x > 639 - 2 * H_OFFSET)
    x = 639 - 2 * H_OFFSET;
  if (x < 0)
    x = 0;
  ...
  if (y > 479 - 2 * V_OFFSET)
    y = 479 - 2 * V_OFFSET;
  drawTopBoundless(x, y, des, mask);
}

// drawTopBoundless(x, y, *des, *mask) will draw, at (x, y), *des with *mask
void drawTopBoundless(short x, short y, const long long* des, const char* mask) {
  int start_pos = 640 * (2 * y + 32 + V_OFFSET) + x * 2 + (32 + H_OFFSET);
  // from stackoverflow thank you
  // mask is 1111 (F) is transparent, 0000 (0) is design
  // need mask to be 32 pixels, 64 bits, duplicated 8 times
  long long full_mask;
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int r = 0; r < 2; r++) {
      full_mask = *mask;
      full_mask = (full_mask | (full_mask << 28)) /* & 0xF0000000F */; // GCC is not able to remove redundant & here
      ...
      // have a mask, need to overlay properly
      memcpy(&copied_original, &vga_data_array[(start_pos + r * 16 + 640 * 2 * i) >> 1], 8); // this copies a full row into copied_design of 32 pixels (64 bits, 8 bytes)
...
// drawHearts(curr, max, *full_des, *emp_des) draws a max of 10 curr hearts in the full_des design
// and draws the difference between max and curr as emp_des on a set location
void drawHearts(short curr, short max, const long long* full_des, const long long* emp_des) {
  int start_pos = 640 * (V_OFFSET + 10) + 364 + H_OFFSET;
  for (int j = 0; j < curr; j++) {
    start_pos += 16;
    for (int i = 0; i < 8; i++) {
      memcpy(&vga_data_array[(start_pos + (2 * i * 640)) >> 1], full_des + i, 8);
      memcpy(&vga_data_array[(start_pos + ((2 * i) + 1) * 640) >> 1], full_des + i, 8);
    }
  }
  for (int j = curr; j < max; j++) {
    start_pos += 16;
    for (int i = 0; i < 8; i++) {
      memcpy(&vga_data_array[(start_pos + (2 * i * 640)) >> 1], emp_des + i, 8);
      memcpy(&vga_data_array[(start_pos + ((2 * i) + 1) * 640) >> 1], emp_des + i, 8);
    }
  }
}

// drawItemSpace() draws the rectangle around the item. Coordinates are hard coded. Gap in between is 13 blocks.
void drawItemSpace(const long long* corner_des, const long long* row_des) {
  int start_pos = 640 * (V_OFFSET - 7) + 44 + H_OFFSET;
  for (int i = 0; i < 2; i++) {
    memcpy(&vga_data_array[(start_pos + (640 * i)) >> 1], corner_des, 24);
    memcpy(&vga_data_array[(start_pos + (640 * (i + 32))) >> 1], corner_des, 24);
    memcpy(&vga_data_array[(start_pos + (640 * (i + 2))) >> 1], row_des, 24);
    memcpy(&vga_data_array[(start_pos + (640 * (i + 30))) >> 1], row_des, 24);
  }
  short bar = 0x6666;
  for (int i = 4; i < 30; i++) {
    memcpy(&vga_data_array[(start_pos - 2 + (640 * i) >> 1)], &bar, 2);
    memcpy(&vga_data_array[(start_pos + 46 + (640 * i) >> 1)], &bar, 2);
  }
}
